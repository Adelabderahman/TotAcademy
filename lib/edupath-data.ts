...
export const levelModules: Record<string, ModuleData[]> = {
  foundation: [
    ...
    { id: "module_ab06f812", num: "09", icon: "🪂", gradient: "linear-gradient(135deg, #2b5876, #4e4376)", moduleLabel: { ar: "مقياس 09", en: "Module 09", fr: "Module 09" }, title: { ar: "الخلاصة التدريبية", en: "Training Summary", fr: "Synthèse de Formation" }, desc: { ar: "ملخص شامل عن جميع المقاييس التدريبية وتجميع المكتسبات المعرفية والتطبيقية في مسار احترافي.", en: "A comprehensive summary of all modules consolidating knowledge and practice.", fr: "Un résumé complet de tous les modules consolidant la théorie et la pratique." } }
  ],
  empowerment: [
...
export const allLessonsDB: Record<string, LessonData[]> = {
  ...
  module_ab06f812: generateModuleLessons('ax9', { ar: "الخلاصة والدمج الاحترافي", en: "Synthesis & Professional Integration", fr: "Synthèse et Intégration" }, 6),
...
export const finalExamQuestions: Record<string, LocalizedString[]> = {
  m1: [
    { ar: "1. كيف توظف مبدأ 'الحاجة إلى المعرفة' عند تصميم دورة لجمهور ذي خبرة عملية واسعة؟ *", en: "1. How do you apply the 'need to know' principle when designing a course for experienced audiences? *", fr: "1. Comment appliquez-vous le principe du 'besoin de savoir' pour un public expérimenté ? *" },
    { ar: "2. اقترح استراتيجية تطبيقية للتعامل مع انخفاض الانتباه بعد أول 20 دقيقة من الجلسة التدريبية. *", en: "2. Suggest a practical strategy to handle attention drops after the first 20 minutes of a session. *", fr: "2. Suggérez une stratégie pour gérer la baisse d'attention après les 20 premières minutes. *" },
    { ar: "3. كيف تدمج تقنيات 'التكرار المتباعد' و'الاسترجاع النشط' في تدريب ممتد لضمان ترسيخ المهارات؟ *", en: "3. How do you integrate 'spaced repetition' and 'active recall' in extended training to anchor skills? *", fr: "3. Comment intégrez-vous la 'répétition espacée' et le 'rappel actif' pour consolider les acquis ? *" },
    { ar: "4. كيف تعدل أسلوبك البيداغوجي إذا أظهر التشخيص القبلي أن أغلبية المتدربين ينتمون للنمط 'الحركي'؟ *", en: "4. How do you adapt your pedagogy if pre-assessment reveals that the majority are 'kinesthetic' learners? *", fr: "4. Comment adaptez-vous votre pédagogie pour une majorité d'apprenants 'kinesthésiques' ? *" },
    { ar: "5. حلل دور 'التغذية الراجعة التكوينية' كأداة للتقييم وتأثيرها المباشر على الدافعية الذاتية للمتدربين. *", en: "5. Analyze the role of 'formative feedback' as an assessment tool and its direct impact on trainee motivation. *", fr: "5. Analysez le rôle du 'feedback formatif' et son impact direct sur la motivation intrinsèque des apprenants. *" }
  ],
  m2: [
    { ar: "1. كيف توظف لغة الجسد وتوزيع النظرات بنظام W لبناء هيبة وألفة فورية في القاعة؟ *", en: "1. How do you use body language and W-pattern eye contact to establish immediate presence and rapport? *", fr: "1. Comment utilisez-vous le langage corporel et le balayage visuel en W pour créer présence et confiance ? *" },
    { ar: "2. حدد آليات التحكم في طبقات الصوت والوقفات الدرامية لكسر الرتابة وإبراز الرسائل الجوهرية. *", en: "2. Detail voice pitch control and dramatic pauses to break monotony and highlight key takeaways. *", fr: "2. Détaillez le contrôle vocal et les pauses dramatiques pour rompre la monotonie et marquer les messages clés. *" },
    { ar: "3. كيف تبني وتوظف قصة تدريبية متكاملة الأركان (بطل، صراع، حل) لترسيخ مفهوم إداري معقد؟ *", en: "3. How do you structure an applied training story (hero, conflict, resolution) to anchor a complex concept? *", fr: "3. Comment structurez-vous une histoire de formation (héros, conflit, résolution) pour ancrer un concept complexe ? *" },
    { ar: "4. اشرح منهجية التعامل الدبلوماسي الحازم مع المتدرب 'المتعالي' دون كسر هيبة القاعة أو إحراجه. *", en: "4. Explain the diplomatic strategy to handle an 'arrogant' participant without derailing the room. *", fr: "4. Expliquez la stratégie diplomatique pour recadrer un participant 'arrogant' avec fermeté et respect. *" },
    { ar: "5. ما خطوات تدريب النفس على التغلب على رهبة المسرح قبل الصعود للمنصة بـ 10 دقائق؟ *", en: "5. What are the psychological steps to manage stage fright 10 minutes prior to stepping on stage? *", fr: "5. Quelles sont les étapes psychologiques pour gérer le trac 10 minutes avant de monter sur scène ? *" }
  ],
  m3: [
    { ar: "1. قارن بدقة بين مراحل نموذج ADDIE الخمس ونماذج التصميم السريع الحديثة. *", en: "1. Rigorously compare the 5 stages of ADDIE with modern rapid instructional design models. *", fr: "1. Comparez rigoureusement les 5 phases d'ADDIE avec les modèles modernes de design rapide. *" },
    { ar: "2. صغ هدفاً تدريبياً مركباً يستوفي شروط SMART ويستهدف المستوى الإبداعي في تصنيف بلوم. *", en: "2. Formulate a SMART training objective targeting the 'Create' tier in Bloom's taxonomy. *", fr: "2. Formulez un objectif SMART ciblant le niveau 'Création' de la taxonomie de Bloom. *" },
    { ar: "3. كيف توظف استراتيجية 'التجزئة' (Chunking) لهندسة حقيبة تدريبية مكثفة من 3 أيام؟ *", en: "3. How do you apply chunking to architect an intensive 3-day training package? *", fr: "3. Comment appliquez-vous le chunking pour concevoir un kit de formation intensif de 3 jours ? *" },
    { ar: "4. ما العناصر الإلزامية التي يجب تضمينها في دليل المدرب (Trainer Manual) لضمان قابلية تشغيله؟ *", en: "4. What mandatory components must be included in a Trainer Manual to guarantee turn-key execution? *", fr: "4. Quels éléments obligatoires doivent figurer dans le Manuel du Formateur pour en garantir l'exécution ? *" },
    { ar: "5. صمم سلم تقييم (Rubric) من 4 مستويات لقياس كفاءة إعداد عرض تقديمي فعال. *", en: "5. Design a 4-tier evaluation rubric to assess competency in delivering an effective presentation. *", fr: "5. Concevez une grille d'évaluation à 4 niveaux pour mesurer la compétence de présentation. *" }
  ],
  m4: [
    { ar: "1. كيف تطبق ميكانيكا التلعيب (نقاط، أوسمة، مسارات) في برنامج مخصص للقيادات العليا؟ *", en: "1. How do you implement gamification mechanics (points, badges, leaderboards) in an executive program? *", fr: "1. Comment appliquez-vous la ludification (points, badges, classements) pour des cadres supérieurs ? *" },
    { ar: "2. حدد معايير صياغة مشكلة واقعية محفزة لتطبيق استراتيجية التعلم القائم على المشكلات (PBL). *", en: "2. Define the criteria for formulating an authentic problem for Problem-Based Learning (PBL). *", fr: "2. Définissez les critères d'un problème authentique pour l'apprentissage par résolution de problèmes (PBL). *" },
    { ar: "3. كيف تصمم سيناريو محاكاة واقعي متعدد المسارات (Branching Scenario) لتدريب خدمة العملاء؟ *", en: "3. How do you design a realistic branching simulation scenario for customer service training? *", fr: "3. Comment concevez-vous un scénario de simulation à embranchements pour le service client ? *" },
    { ar: "4. ما الضوابط التي تمنع تحول الترفيه التعليمي (Edutainment) إلى مضيعة للوقت بلا عائد معرفي؟ *", en: "4. What safeguards ensure edutainment does not devolve into frivolous time-wasting without learning? *", fr: "4. Quels garde-fous empêchent l'edutainment de dériver en divertissement stérile sans apprentissage ? *" },
    { ar: "5. وضح بالخطوات كيفية توظيف التفكير التصميمي (Design Thinking) لحل مشكلة تسرب المتدربين. *", en: "5. Step-by-step, explain using Design Thinking to solve the challenge of trainee drop-out. *", fr: "5. Détaillez par étapes l'usage du Design Thinking pour résoudre l'abandon des apprenants. *" }
  ],
  m5: [
    { ar: "1. كيف تدمج نماذج الذكاء الاصطناعي التوليدي في مرحلة إعداد السيناريوهات والأنشطة التدريبية؟ *", en: "1. How do you integrate generative AI models in creating training scenarios and activities? *", fr: "1. Comment intégrez-vous l'IA générative dans la création de scénarios et d'activités pédagogiques ? *" },
    { ar: "2. قارن بين منصات LMS التقليدية ومنصات تجربة التعلم الحديثة (LXP) من حيث تفاعل المتعلم. *", en: "2. Contrast traditional LMS platforms with modern LXPs regarding learner engagement. *", fr: "2. Comparez les LMS traditionnels et les LXP modernes en termes d'engagement des apprenants. *" },
    { ar: "3. ما هي البروتوكولات الأمنية والاحترازية لتأمين قاعة تدريب افتراضية وحماية بيانات المشاركين؟ *", en: "3. What security protocols are required to safeguard a virtual classroom and participant data? *", fr: "3. Quels protocoles de sécurité sont nécessaires pour protéger une classe virtuelle et les données ? *" },
    { ar: "4. كيف توظف أدوات الأتمتة (No-Code/Zapier) لبناء رحلة متابعة مستمرة بعد انتهاء التدريب؟ *", en: "4. How do you deploy no-code automation (Zapier) to build an ongoing post-training follow-up workflow? *", fr: "4. Comment déployez-vous l'automatisation sans code (Zapier) pour structurer le suivi post-formation ? *" },
    { ar: "5. حدد المعايير الأخلاقية الواجب التزامها عند إدراج محتوى مولد بالذكاء الاصطناعي في دوراتك. *", en: "5. Outline the ethical guidelines required when incorporating AI-generated content in your courses. *", fr: "5. Exposez les règles éthiques à respecter lors de l'intégration de contenu généré par l'IA. *" }
  ],
  m6: [
    { ar: "1. اشرح بالتفصيل معادلة العائد على الاستثمار (ROI) وفق منهجية جاك فيليبس موضحاً التكاليف الصافية. *", en: "1. Detail the Jack Phillips ROI equation, elaborating on fully-loaded costs vs net monetary benefits. *", fr: "1. Détaillez l'équation du ROI de Jack Phillips, en expliquant les coûts complets vs bénéfices nets. *" },
    { ar: "2. ما هي الآليات الإحصائية المعتمدة لعزل أثر التدريب عن المؤثرات الخارجية الأخرى في السوق؟ *", en: "2. What statistical mechanisms isolate training effects from external market fluctuations? *", fr: "2. Quels mécanismes statistiques permettent d'isoler l'impact de la formation des facteurs externes ? *" },
    { ar: "3. كيف تحول مخرجات المهارات الناعمة (مثل إدارة الضغوط) إلى قيم مالية قابلة للقياس الكمي؟ *", en: "3. How do you monetize soft skill outcomes (e.g. stress resilience) into quantifiable currency? *", fr: "3. Comment monétisez-vous les soft skills (ex: gestion du stress) en valeurs financières quantifiables ? *" },
    { ar: "4. صمم لوحة قيادة افتراضية (Dashboard) تتضمن 5 مؤشرات حيوية لقياس أثر برنامج تدريبي للقيادة. *", en: "4. Design a mock dashboard with 5 core KPIs to measure the impact of a leadership program. *", fr: "4. Concevez un tableau de bord avec 5 KPI vitaux pour mesurer l'impact d'un programme de leadership. *" },
    { ar: "5. كيف تقدم تقريراً تنفيذياً للإدارة العليا يقنعهم بزيادة ميزانية التدريب السنوية بناءً على الأرقام؟ *", en: "5. How do you pitch an executive impact report to senior leaders to justify budget expansion based on data? *", fr: "5. Comment présentez-vous un rapport d'impact exécutif pour justifier l'extension budgétaire par les chiffres ? *" }
  ],
  m7: [
    { ar: "1. ما هي مرتكزات الحوكمة الرشيدة للمؤسسات التدريبية لضمان الاستدامة التشغيلية والمالية؟ *", en: "1. What are the foundations of good governance for training institutions to ensure operational sustainability? *", fr: "1. Quels sont les piliers d'une bonne gouvernance garantissant la pérennité opérationnelle et financière ? *" },
    { ar: "2. حلل مراحل الحصول على اعتماد معايير الجودة الدولية (مثل ISO 10015) الخاصة بإدارة التدريب. *", en: "2. Analyze the milestones to obtain international ISO 10015 quality accreditation for training management. *", fr: "2. Analysez les étapes d'obtention de l'accréditation qualité internationale ISO 10015. *" },
    { ar: "3. كيف تبني مصفوفة الصلاحيات والمسؤوليات (RACI) لإدارة مشروع تدريبي استراتيجي ضخم؟ *", en: "3. How do you construct a RACI matrix to govern a large-scale strategic training deployment? *", fr: "3. Comment construisez-vous une matrice RACI pour piloter un projet de formation stratégique majeur ? *" },
    { ar: "4. ما استراتيجيات تسعير الحقائب والخدمات التدريبية الموجهة للشركات الكبرى (B2B Pricing)؟ *", en: "4. What are the core B2B value-based pricing strategies for corporate training contracts? *", fr: "4. Quelles sont les stratégies de tarification B2B basée sur la valeur pour les grands comptes ? *" },
    { ar: "5. كيف تبني شراكات استراتيجية مع الجامعات والمراكز الدولية تعزز الموثوقية وتفتح أسواقاً جديدة؟ *", en: "5. How do you build strategic partnerships with universities to boost credibility and open new markets? *", fr: "5. Comment bâtir des partenariats stratégiques universitaires pour asseoir la crédibilité et conquérir des marchés ? *" }
  ],
  m8: [
    { ar: "1. حدد بدقة مفهوم التخصص الدقيق (Niche) وكيف يصنع للمدرب مكانة ريادية لا يمكن منافستها. *", en: "1. Define deep niche positioning and how it establishes an unassailable leadership authority for a trainer. *", fr: "1. Définissez le positionnement de niche et comment il forge une autorité incontestable pour le formateur. *" },
    { ar: "2. ما هي استراتيجية صناعة المحتوى على منصة LinkedIn لتحويل المتابعين إلى عقود تدريبية واستشارية؟ *", en: "2. What is the LinkedIn content strategy to convert professional followers into high-ticket contracts? *", fr: "2. Quelle stratégie de contenu LinkedIn convertit les abonnés professionnels en contrats à haute valeur ? *" },
    { ar: "3. كيف توظف الدليل الاجتماعي (Social Proof) وقصص نجاح الخريجين لبناء سمعة مهنية ذات موثوقية؟ *", en: "3. How do you deploy social proof and alumni success stories to cement an authoritative reputation? *", fr: "3. Comment déployez-vous la preuve sociale et les réussites d'anciens pour asseoir votre réputation ? *" },
    { ar: "4. وضح آليات حماية الملكية الفكرية لحقائبك ومؤلفاتك التدريبية وصياغة عقود الترخيص (Licensing). *", en: "4. Detail IP protection mechanisms for training packages and structuring IP licensing contracts. *", fr: "4. Détaillez les mécanismes de protection de la propriété intellectuelle et la structuration de contrats de licence. *" },
    { ar: "5. ما هي خارطة الطريق لتحويل خبرتك كمدرب من الاعتماد على الوقت إلى بناء أصول رقمية مدرة للدخل؟ *", en: "5. What is the roadmap to transition from trading time for money to building scalable digital revenue assets? *", fr: "5. Quelle feuille de route permet de passer de la vente de son temps à la création d'actifs numériques rentables ? *" }
  ]
};
